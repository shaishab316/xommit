...
import { displayMessage, displayDiff, confirmCommit, showSpinner, initPrompt } from './ui';
...
export async function runCommit(options: CommitOptions = {}): Promise<void> {
  try {
    ...
    const { hasChanges, stagedFiles, unstagedFiles, newFiles } = await checkGitChanges();
    spinner.stop();
    ...
    const { confirmed, shouldPush, shouldStageAll } = await confirmCommit(options);
